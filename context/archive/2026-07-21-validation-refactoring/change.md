...
- **FilesTrigger snapshot init gate (post Phase 6):** `a62aa06` — `hasSnapshot` requires every entry to have boolean `isFolder`. Invalid/legacy entries fail the init gate and re-seed (no emit) so `getSnapshot` cannot drop keys that `classifyDirectoryChanges` would treat as creates.
