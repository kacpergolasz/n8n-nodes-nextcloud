...
- **Reuse `nodes/shared/pollOrchestration.ts`** — lifecycle envelopes shared by Files + News (and future app triggers):
  ...
  - `handlePollListingFailure` — after init, soft-fail via policy hook (`silent` → `null`, or `oneShotNotice` → at most one `{ event: 'pollError', message }` per failure window); before init, rethrow. Caller passes `error` + app `scrubError`; the helper scrubs before log/notice/throw. Clear the notice flag with `clearSoftFailNotice` on the next successful listing.
...
| Steady catch-up | Node params `pageSize` (default **100**) and `maxPagesPerPoll` (default **5**). Newest→older walk toward `maxProcessedId` until id ≤ watermark, empty/exhausted list, or page cap. Cap persists `catchUpOffset` (resume survives soft-fail); raise watermark only after catch-up completes. Ring buffer dedupes only — it does not end catch-up. Emit new ids ascending. The node enforces `minValue: 1` only — upper bounds are left to the Nextcloud News server / operator (oversized pages may fail or be truncated server-side). **Known limitation:** while catch-up is incomplete, W is frozen and only the ~10k ring dedupes already-emitted ids — operators must keep poll capacity (`pageSize × maxPagesPerPoll` × poll frequency) ahead of ingest so catch-up completes; otherwise extreme sustained backlog can re-emit after ring eviction. |
